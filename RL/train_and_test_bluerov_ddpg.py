...
def parse_args():
    parser = argparse.ArgumentParser(description="Train or test DDPG on BlueROVEnv")
...
def train_model(train_seed, total_timesteps):
    ...
    print(f"🚀 Entraînement DDPG du BlueROV, dossier de sauvegarde : {savedir}")
    ...
    model.save(os.path.join(savedir, "final_model_bluerov_ddpg"))
    train_env.save(os.path.join(savedir, "vecnormalize_ddpg.pkl"))
    print("✅ Modèle entraîné et sauvegardé.")
    train_env.close()

def test_model(test_seed, num_episodes):
    savedir = get_savedir("test")
    print(f"🧪 Test du modèle DDPG sur BlueROV, dossier utilisé : {savedir}")

    test_env = DummyVecEnv([lambda: Monitor(BlueROVEnv(seed=test_seed, save_dir=savedir, mode="test"))])
    test_env = VecNormalize.load(os.path.join(savedir, "vecnormalize_ddpg.pkl"), venv=test_env)
    model = DDPG.load(os.path.join(savedir, "final_model_bluerov_ddpg.zip"), device="cpu")
    print(model.policy)
